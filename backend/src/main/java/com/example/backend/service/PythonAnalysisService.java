...
public class PythonAnalysisService { // 클래스 이름 변경
     ...
     * @param counselorName
     */
    public void analyzeConsultation(Long counselId, String rawText, String counselorName) {
        logger.info("파이썬 분석 서비스 호출 중. URL: {}", pythonAnalysisApiUrl);

        // 파이썬 서비스에 보낼 요청 바디 (JSON)
        // 파이썬 서비스는 이 JSON을 받아 분석을 시작합니다.
        String requestBody = String.format("{\"counsel_id\": \"%s\", \"raw_text\": \"%s\", \"counselor_name\": \"%s\"}",
                String.valueOf(counselId), escapeJson(rawText), escapeJson(counselorName));
        ...
        try {
            ...
            webClient.post()
                    ...
                    .subscribe(
                            successBody -> logger.info("파이썬 서비스 응답: {}", successBody),
                            error -> logger.error("파이썬 서비스 호출 실패: {}", error.getMessage(), error)
                    );
